...
/*   Updated: 2019/10/13 22:28:12 by laleta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lem_visual.h"

static int8_t	init_pic_room(t_pic_room *proom, int32_t radius)
{
	if (!(proom->circle = sfCircleShape_create()) ||
		!(proom->text = sfText_create()) ||
		!(proom->font = sfFont_createFromFile(LM_FONT_ROOM)))
		return (0);
	sfCircleShape_setRadius(proom->circle, radius);
	sfText_setFont(proom->text, proom->font);
	sfText_setCharacterSize(proom->text, radius * 1.7);
...
int8_t			set_backgr_2d(t_world *world, t_room *room_list, int32_t i)
{
	...
	if (!init_pic_room(&proom, world->room_radius * 400))
		return (0);
	...
	while (i < 2 * world->link_cnt)
	{
		...
		draw_link_2d(world, p1, p2, world->link_thick);
		i += 2;
	}
